...
export const useFileUpload = (
  ...
  const clearFiles = useCallback(() => {
    ...
    // onFilesChangeはRHFのfield.onChange経由で別コンポーネントのstateを更新するため、
    // setStateのアップデータ内(レンダーフェーズ扱い)ではなく呼び出し元のイベントハンドラ内で呼ぶ
    onFilesChange?.([]);
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      if (validFiles.length > 0) {
        ...
        // onFilesChangeはsetStateのアップデータ内(レンダーフェーズ扱い)ではなく
        // イベントハンドラ内で呼ぶ(別コンポーネントのstate更新を安全に行うため)
